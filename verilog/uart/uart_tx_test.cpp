...
void opentrace(const char *vcdname, Vuart_tx *core) {
...
static PT_THREAD(actions_pt(struct pt *pt)) {
    ...
    top->i_reset = 0;
    PT_WAIT_UNTIL(pt, posedge());
    top->i_cyc = top->i_we = 1; top->i_dat = 0x47;
    PT_WAIT_UNTIL(pt, posedge());
    top->i_cyc = top->i_we = 0;
    PT_WAIT_UNTIL(pt, posedge());
    PT_WAIT_UNTIL(pt, top->uart_tx__DOT__state_tx == 11);
    PT_END(pt);
}

int main(int argc, char** argv) {
    cout << "Simulating uart_tx..." << endl;
